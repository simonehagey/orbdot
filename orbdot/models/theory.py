...
def companion_rv_trend_from_mass(tau, M_c, M_s):
    ...
    The above equation assumes that the companion's orbit has an eccentricity of 0.5,
    an argument of pericenter that is exactly 90 degrees, and a period that is 1.25 times the time
    span of the observations. In this scenario, the observed linear trend in the radial velocity
    model is a segment of the sawtooth-like curve of the companion planet (see Figure 1 of [1]_),
    for which the semi-amplitude can be approximated as half of the baseline multiplied by the
    acceleration (:math:`0.5 * \\tau * \\dot{\\gamma}`).
...
def companion_from_quadratic_rv(P_min, t_pivot, dvdt, ddvdt, M_s):
    ...
    -------
    tuple
        The minimum possible orbital period in days, the associated lower limit on the RV semi-
        amplitude in m/s, the companion mass in Earth masses, and the time when the outer
